package com.github.xplosunn.tapirgen.language

import com.github.xplosunn.tapirgen.EndpointToGenerated.{Function, HttpCall, OutcomeOfPathElement, Parameter}
import com.github.xplosunn.tapirgen.{HttpCaller, Language, TypeDeclaration}

trait Scala

case object Scala extends Scala {
  implicit val language = new Language[Scala] {
    override def generateCode(packageName: String,
                               apiName: String,
        ...
        .map {
          ...
          case Function(name, impl) =>
            ...
            s"""class $name(basePath: String) {
               ...
               |  new $name(basePath + "/" + $name)
               |""".stripMargin.indent(indentationLevel)
          case httpCall @ HttpCall(_, _, _, _) =>
