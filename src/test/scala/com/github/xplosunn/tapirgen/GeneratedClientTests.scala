...
import tapir.{Endpoint, endpoint, header, jsonBody, path, query, stringBody}
...
class GeneratedClientTests extends FlatSpec with Matchers {
  ...
  it should "generate sample call" in {
    ...
    val generatedCode = new EndpointToGenerated("sample.tapir.generated", Sttp).generate("bookApi", booksListing)

    val expectedCode =
      """package sample.tapir.generated
        |
